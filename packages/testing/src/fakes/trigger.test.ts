...
import { FakeTriggerApiError, FakeTriggerClient, parseTriggerDuration } from "./trigger.ts";
...
describe("FakeTriggerClient tasks", () => {
  ...
  it("deduplicates on idempotency keys per task until the key's TTL passes", async () => {
    ...
    await trigger.idempotencyKeys.reset("search-index", "search:u1:1");
  ...
  it("runs triggerAndWait children in process, marks the parent WAITING, and returns results", async () => {
    ...
    await expect(trigger.tasks.triggerAndWait("unregistered", {})).rejects.toMatchObject({
      status: 404,
    });
  });
  ...
  it("batch-triggers runs", async () => {
    ...
    expect(batch.batchId).toBe("batch_fake000001");
