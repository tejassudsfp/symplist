...
describe("the search archive contributor (§2.1)", () => {
  it("is registered and inserts one intent per archived task, guarded by the archive write id", async () => {
    ...
    await store.db.batch(searchArchiveContributor.statements(input));
    ...
    await store.db.batch([
      ...
      ...searchArchiveContributor.statements(input),
    ]);
    const rows = await store.db.all(
      sql(
        `SELECT entity, entity_id, op FROM search_intents WHERE owner_id = :owner ORDER BY entity_id`,
        {
          owner,
        },
      ),
    ...
    expect(rows).toEqual(
      [root, child].sort().map((id) => ({ entity: "task", entity_id: id, op: "upsert" })),
    );
