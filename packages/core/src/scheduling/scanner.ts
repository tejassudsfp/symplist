...
export class ReminderScanner {
  ...
  async run(input: ScannerExecution) {
    ...
    for (const row of rows) {
      ...
      await this.materialize(String(row.id), input);
      occurrenceCount++;
    }
    ...
    for (const row of due) {
      if (input.signal?.aborted) break;
      if (await this.deliver(String(row.id), input)) acceptedCount++;
    }
  ...
  private async materialize(id: string, input: ScannerExecution) {
    ...
    const [_, claimed] = await this.options.db.batch([
      ...
      sql(
        ...
        p.zone,p.default_hour,p.in_app,p.email AS email_enabled,p.quiet_enabled,p.quiet_start,p.quiet_end,p.email_preview,
        ...
        (SELECT COUNT(*) FROM reminder_occurrences prior JOIN reminders pr ON pr.id=prior.reminder_id WHERE prior.owner_id=o.owner_id AND prior.task_id=o.task_id AND prior.intended_at<o.intended_at AND prior.status IN ('pending','claimed') AND pr.generation=prior.generation AND pr.status='active' AND pr.channels_json LIKE '%in_app%') AS missed_count
    ...
    const guard = `EXISTS(SELECT 1 FROM reminder_occurrences WHERE id=:occurrence AND lease_owner=:lease AND fence=CAST(:fence AS INTEGER) AND lease_until>:now AND status='claimed') AND ${this.executorGuard()}`;
    const params = {
      occurrence: id,
      lease,
      fence: int(fence),
    ...
    try {
      ...
      if (channels.includes("in_app") && prefs.inApp) {
        ...
        statements.push(
          ...
          AND NOT EXISTS(SELECT 1 FROM reminder_occurrences newer JOIN reminders nr ON nr.id=newer.reminder_id WHERE newer.owner_id=:owner AND newer.task_id=:task AND newer.intended_at>CAST(:intended AS INTEGER) AND newer.intended_at<=:now AND newer.status IN ('pending','claimed','delivered') AND nr.status='active' AND nr.generation=newer.generation AND nr.channels_json LIKE '%in_app%')
      ...
      if (
        ...
        const payload = await this.options.renderEmail({
          ownerId: owner,
          taskId: task,
          occurrenceId: id,
          title,
        ...
        statements.push(
          ...
          AND NOT EXISTS(SELECT 1 FROM reminder_occurrences newer JOIN reminders nr ON nr.id=newer.reminder_id WHERE newer.owner_id=:owner AND newer.task_id=:task AND newer.intended_at>CAST(:intended AS INTEGER) AND newer.intended_at<=:now AND newer.status IN ('pending','claimed','delivered') AND nr.status='active' AND nr.generation=newer.generation AND nr.channels_json LIKE '%email%') ON CONFLICT(occurrence_id,channel) DO NOTHING`,
            {
              ...params,
              ...liveParams,
              id: outboxId,
      ...
      if (result.at(-1)?.results.length && !quiet)
        await this.options.notify?.(owner, notificationId);
    } finally {
      zeroize(key.key);
    }
  }
  private async deliver(id: string, input: ScannerExecution): Promise<boolean> {
    ...
    const [_, result] = await this.options.db.batch([
      ...
      sql(
        `SELECT b.*,o.intended_at,o.generation AS reminder_generation,r.generation AS current_generation,r.status AS reminder_status,t.status AS task_status,u.email,p.email AS email_enabled,k.kek_version,k.wrapped_key
    ...
    const live = await this.options.db.first(
      ...
      AND EXISTS(SELECT 1 FROM reminders r JOIN reminder_occurrences o ON o.reminder_id=r.id WHERE o.id=notification_outbox.occurrence_id AND r.status='active' AND r.generation=o.generation)
      AND NOT EXISTS(SELECT 1 FROM email_suppressions WHERE ${suppression})`,
        { ...params, now: int(this.options.now()), owner, ...suppressionParams },
      ),
    );
    if (!live || input.signal?.aborted) {
      await this.options.db.run(
        sql(
          `UPDATE notification_outbox SET status='cancelled',write_id=:lease WHERE ${guard}`,
          params,
        ),
      );
      return false;
  ...
  async quietSummaries(input: ScannerExecution): Promise<void> {
    ...
    const owners = await this.options.db.all(
      sql(
        `SELECT * FROM notification_prefs p WHERE quiet_enabled=1 AND EXISTS(SELECT 1 FROM notifications n WHERE n.owner_id=p.owner_id AND n.quiet=1 AND n.intended_at>=COALESCE(p.last_quiet_summary_at,0) AND n.read_at IS NULL AND n.dismissed_at IS NULL) AND ${this.executorGuard()} ORDER BY owner_id LIMIT 50`,
        this.executionParams(input),
      ),
    );
    for (const row of owners) {
      ...
      const owner = String(row.owner_id);
      const w = uuidv7(now);
      const params = { owner, w, end: int(end.epochMilliseconds), ...this.executionParams(input) };
      const result = await this.options.db.batch([
        sql(
          `UPDATE notification_prefs SET last_quiet_summary_at=:end,write_id=:w WHERE owner_id=:owner AND (last_quiet_summary_at IS NULL OR last_quiet_summary_at<CAST(:end AS INTEGER)) AND ${this.executorGuard()} AND ${this.service.access()}`,
          params,
        ),
        sql(
          `SELECT COUNT(*) AS count FROM notifications WHERE owner_id=:owner AND quiet=1 AND intended_at>=CAST(:start AS INTEGER) AND intended_at<CAST(:end AS INTEGER) AND read_at IS NULL AND dismissed_at IS NULL AND EXISTS(SELECT 1 FROM notification_prefs WHERE owner_id=:owner AND write_id=:w)`,
