...
| [07_architecture.md](../notes/files/07_architecture.md) | In progress | Foundation executors and Simon persistence (`packages/core/src/simon/`); model/tool execution and application integration remain | `repository.test.ts`: duplicate submissions, ownership, encryption, stop, queued advancement, executor generation and restriction/purge |
...
| [12_simon_meta_tools.md](../notes/files/12_simon_meta_tools.md) | In progress | `packages/agent/src/policy.ts`, `rules.ts`; wrappers and runtime tool integration remain | `policy.test.ts`: conservative approvals, schema changes, mixed batches, forbidden tools and escaped untrusted data |
