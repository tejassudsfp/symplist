...
- **Contract suite.** The same suite runs against both clients: local always, live when `LIVE_D1=1`. The live part covers batch rollback when statement 2 fails, `RETURNING` population, number, boolean and null params, the batch error shape (HTTP 400 versus 200 with `success: false`), and records rate-limit header availability. When both `Ratelimit` headers are present it measures whether the budget is per token, per user or per account; when the live D1 endpoint omits both, the result is explicitly `unknown` and the conservative shared budget remains binding.
...
- `syncEnvVars` in `trigger.config.ts` reads the worker allowlist from `@symplist/config/worker`, marks secrets `isSecret: true`, and never syncs anything else. A credentialed CLI deployment that presents any allowlisted value must present a complete valid worker environment and syncs it. A repository-linked build receives no runtime secrets and returns an empty sync set, preserving the environment already managed by Trigger; this boundary prevents secrets entering the GitHub image builder without making linked builds fail. The first deployment and every environment change therefore use the credentialed CLI path. Variables removed from the allowlist are deleted from Trigger by hand, because sync never deletes.
...
- Connector results, MCP-authored document content and Composio search guidance enter the model context inside fixed delimiters, `<untrusted_data source="connector|document|composio" ref="<id>">…</untrusted_data>`, with any closing delimiter inside the content escaped, and a runtime-injected rule that instructions inside these blocks are data. Task documents are never a source of rules. Provider-controlled Composio output has a cumulative 96 KiB model-visible budget per run/effect; later output becomes the fixed `integration.result_budget_exhausted` sentinel rather than enlarging the prompt or being retried.
...
- Loop: `instructions` (not `system`), `await convertToModelMessages(await validateUIMessages(...))`, `stopWhen: [isStepCount(10), cumulativeOutputBudget, approvalPause]`, `maxOutputTokens` at most 4,096 per step and dynamically reduced so a run cannot request more than 8,192 output tokens in total, `abortSignal` wired to stop, `parallelToolCalls: false`, `telemetry: { isEnabled: false }`, `toUIMessageStream({ sendReasoning: false })`, and an `onError` that records only a stable code. The newest model history suffix is at most 40 messages and 64 KiB (always retaining the current message); advancing its actual floor increments `context_epoch` exactly once.
- OpenAI GPT-5.6 uses implicit 30-minute prompt-prefix caching. No cache key, response memoization or `previous_response_id` is used: prior turns remain encrypted Symplist history, and permissions/task revisions are checked afresh. A credential-gated live contract repeats an eligible prefix and requires a reported cache read.
- Operational telemetry comes only from `onStepEnd` allowlisted numeric fields written to `runs` (§8.1), including input, cache-read, cache-write and output token counts. SDK telemetry remains disabled and no prompt content is recorded.
...
- CI (`.github/workflows/ci.yml`): `verify` (install, `biome ci`, typecheck, test, build) → `e2e` and `api-image` → on `main` only `migrate` (§3.4). Trigger.dev remains repository-linked and auto-deploys from `main`; CI deliberately has no second deploy job.
