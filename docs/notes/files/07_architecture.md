...
- Trigger documents both chat agents backed by sessions and backend-readable session channels. Its chat wrapper can integrate with AI SDK; exact adapter compatibility must be verified against pinned package versions. [Chat backend](https://trigger.dev/docs/ai-chat/backend), [sessions](https://trigger.dev/docs/ai-chat/sessions), [session channels](https://trigger.dev/docs/management/sessions/channels).
