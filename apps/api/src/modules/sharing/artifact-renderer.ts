...
const hash = createHash("sha256").update(ARTIFACT_CSS).digest("base64");
...
function page(title: string, body: string) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><style>${ARTIFACT_CSS}</style></head><body><main>${body}<footer><small>Shared read-only with symplist</small></footer></main></body></html>`;
}
