...
Durable execution adds one exception worth stating plainly. When a deployment runs Simon as a durable chat session, the turn's messages and streamed output pass through the execution provider's realtime streams and remain readable there for that provider's retention window. The conversation itself is stored only in the application's own database under the account key — the provider's default transcript snapshot is replaced, not merely disabled — and account deletion removes it. A deployment that does not enable durable execution has no such exception.
